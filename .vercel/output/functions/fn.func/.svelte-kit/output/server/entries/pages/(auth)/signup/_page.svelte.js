...
<main class="flex w-full lg:h-screen lg:overflow-hidden"><div class="hidden w-1/2 bg-gradient-to-r from-cyan-500 to-blue-500 lg:block"></div>
	<section class="flex h-screen w-full flex-col items-center justify-center gap-7 px-5 md:gap-10 md:px-10 lg:h-full lg:w-1/2 lg:px-0"><div class="flex w-full flex-col gap-5 md:w-2/3 lg:gap-10"><div class="flex items-center gap-5"><a href="/login" class="cursor-pointer">${validate_component(Icon, "Icon").$$render(
    $$result,
    {
    ...
    {}
				...
				<div class="flex flex-col gap-2 text-left"><h1 class="text-3xl font-bold">Hi Newbie!</h1>
					<p class="text-gray-500">Let&#39;s make it quick</p></div></div></div>
    ...
    {}
			...
			<fieldset class="flex flex-col gap-2"><label for="confirmPassword">Confirm password</label>
				<input${spread(
    [
      { type: "password" },
    ...
    {}
			...
			<fieldset class="flex items-center gap-2"><input${spread(
    [
      { type: "checkbox" },
      { name: "privacy" },
      { class: "rounded-sm" },
      escape_object($constraints.privacy)
    ],
    {}
  )}${add_attribute("checked", $form.privacy, 1)}>

				<label for="password">I agree to the
					<a href="https://glowing-brand-015819.framer.app/blog/terms-privacy" class="underline">Terms &amp; Privacy
					</a></label>
				${$errors.privacy ? `<p class="text-sm text-red-500">${escape($errors.privacy)}</p>` : ``}</fieldset>

			<button type="submit" class="w-full rounded-md bg-black p-4 text-white">Sign up</button></form></section></main>`;
});
export {
  Page as default
};
